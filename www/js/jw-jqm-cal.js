         ...
         for ( var i = 0, event; event = plugin.settings.events[i]; i++ ) {
            if ( event[plugin.settings.day] < end && event[plugin.settings.day] >= begin) {
               // Append matches to list
			      var totalHoursAM = timeDiff(event[plugin.settings.beginAM], event[plugin.settings.endAM]);
			      var totalHoursPM = timeDiff(event[plugin.settings.beginPM], event[plugin.settings.endPM]);
			      
			      var dailyHoursWorked = "<div class='detailTimeSheet'><div style='float:left;'><img src='img/chrono.png' width='22' height='22'/><span>" + event[plugin.settings.beginAM] + " - " + event[plugin.settings.endAM] + "</span><br/><img src='img/chrono.png' width='22' height='22'/><span>" + event[plugin.settings.beginPM] + " - " + event[plugin.settings.endPM] + "</span></div><div class='singleton'><img src='img/burger.png' width='32' height='32' /></div><div class='singleton'>" + addDuration(totalHoursAM, totalHoursPM) + "</div></div>";
               $("<li data-icon='nf-arrow-r'><a href='#'>" + dailyHoursWorked + "</a></li>").appendTo($listview);    
            }
         }
